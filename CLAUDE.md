...
There is a second, separate device family: **`print_d80.py`** targets the DP-D80/D80H/PCPS_D80 (Letter/A4 printers), documented in [PROTOCOL_D80.md](PROTOCOL_D80.md). It is a different branch of the LuckPrinter SDK (`BaseA4Device`, not `BaseNormalDevice`) with a different print-width scheme, an extra "set paper type" command, and — critically — **unverified bitmap handling**: the app compresses D80 images with a proprietary native codec (`Compress.codeLihu`) that was not reverse-engineered; `print_d80.py` instead gambles on sending a plain uncompressed `GS v 0` raster like the DP-L1S does, which is untested against real hardware. Don't assume it works without a hardware confirmation from the user.
